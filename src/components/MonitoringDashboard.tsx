...
const MonitoringDashboard: React.FC = () => {
  ...
  const checkAlertConditions = useCallback((metrics: SystemMetrics) => {
    ...
    // レスポンス時間チェック
    if (metrics.averageResponseTime > 1000) { // 1秒以上
      newAlerts.push({
        level: 'warning',
        ...
        {metrics && health && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className={`text-2xl font-semibold ${health.status === 'healthy' ? 'text-green-500' : 'text-red-500'}`}>
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
                稼働時間: {formatUptime(health.uptime)}
              </p>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
                エラー: {metrics.errorCount} ({((metrics.errorCount / Math.max(metrics.requestCount, 1)) * 100).toFixed(1)}%)
              </p>
            </div>
            ...
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {alerts.slice(0, 10).map((alert, index) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center">
                      <span className="font-medium text-sm uppercase tracking-wide mr-2">
                        {alert.level}
                      </span>
                    ...
                    {Object.entries(metrics.endpointAverages)
                      ...
                      .map(([endpoint, stats]) => (
                        <tr key={endpoint}>
                          ...
                          <td className={`px-6 py-4 whitespace-nowrap text-sm ${getStatusColor(stats.errorRate, stats.averageTime)}`}>
                  ...
                  .map(([status, count]) => (
                    <div key={status} className="text-center">
                      <div className={`text-2xl font-bold ${
                        Number(status) >= 400 ? 'text-red-500' : 
                        Number(status) >= 300 ? 'text-yellow-500' : 
                        'text-green-500'
                      }`}>
                        {count}
                      </div>
